...
> 当前阶段：已完成基础初始化与第一步事件规范设计。尚未实现数据采集、ICS 生成、网站界面或自动发布，也没有可用的订阅地址。
...
第一步设计见 [统一事件数据规范 v1](docs/event-model.md)；下一步准备虚构样例。详细范围见 [实施路线](docs/roadmap.md)。当前没有安装、运行或部署命令；业务代码将在方案确认后开始。
